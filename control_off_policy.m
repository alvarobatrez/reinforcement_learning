...
% Política objetivo target_policy: greedy determinística respecto a Q
[~, target_policy] = max(Q, [], 3);
target_policy(M==-2) = 0;
target_policy(M==10) = 0;
...
% Aprende la política óptima target_policy mientras explora con mu
for episode = 1 : num_episodes
    ...
    for t = length(states) : -1 : 1
        ...
        target_policy(states(t, 1), states(t, 2)) = A;

        % Si la acción tomada no es la greedy, ratio de importancia = 0
        % (porque target_policy es determinística: pi(a|s) = 0 si a ≠ greedy)
        if actions_taken(t) ~= target_policy(states(t, 1), states(t, 2))
            break
        end
